...
interface NavbarProps {
  showAuthButtons?: boolean; // optional, default true
  minimal?: boolean; // for login/register pages
}

export default function Navbar({ showAuthButtons = true, minimal = false }: NavbarProps) {
  ...
  return (
    <nav className="fixed top-0 w-full backdrop-blur-md bg-white/70 border-b border-gray-200 z-50">
      <div className="max-w-7xl mx-auto flex justify-between items-center px-6 py-4">
        ...
        {/* Auth Buttons */}
        {showAuthButtons && (
          <div className={`hidden md:flex items-center gap-4 ${minimal ? "ml-auto" : ""}`}>
            <button
